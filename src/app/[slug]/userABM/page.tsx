...
export default async function Page({params}: Props) {
    ...
    if (!currentSchool) return <>Error al obtener el colegii.</>
            ...
            <div className="w-full">
                <table className="w-full">
                    <tbody>
                        ...
                        {users.map((u) => <tr>
                            <td className="border border-black"><Link href={`/${params.slug}/userABM/${u.user.id}`}>{u.user.name}</Link></td>
                            <td className="border border-black"><Link href={`/${params.slug}/userABM/${u.user.id}`}>{u.user.email}</Link></td>
                            <td className="border border-black"><Link href={`/${params.slug}/userABM/${u.user.id}`}>{u.school_user.role}</Link></td>
                            <td className="border border-black"><Link href={`/${params.slug}/userABM/${u.user.id}`}>{u.school_user.isActive ? "alta" : "baja"}</Link></td>
                        </tr>)}
                    </tbody>
                </table>
            </div>
