...
export default async function Page({ params }: Props) {
    ...
    const update = async (data: FormData) => {
        ...
        const newGradeId = gradeiDType.safeParse({ id: Number(data.get("studentGrade")) })
            ...
            <div className="bg-primary-100 flex flex-col gap-5 m-5 p-4 rounded-xl">
                <form action={update}>
                    <div className="grid grid-cols-2 gap-4">
                        <TextField id="" name="studentFirstName" label="Nombre" defaultValue={currentStudent.student.firstName} required />
                        <TextField id="" name="studentLastName" label="Apellido" defaultValue={currentStudent.student.lastName} required />
                        <TextField id="" name="studentEmail" label="E-mail" defaultValue={currentStudent.student.email} required />
                        <TextField id="" name="studentCode" label="Legajo" defaultValue={currentStudent.student.studentCode} required />
                        <div className="flex-none basis-1/2 grow w-full">
                            <label htmlFor="" className="mt-4">Curso*</label>
                            <select name="studentGrade" defaultValue={currentStudent.student_grade.gradeId} className="outline outline-1 outline-outline rounded p-4 w-full">
                                <option value="">----</option>
                                {grades.map((c) => <option value={Number(c.id)}>{c.name}</option>)}
                            </select>
                        </div>
                    </div>
                    <label htmlFor="" className="text-xl basis-full">Padre/Madre/Tutor</label>
                    <input type="hidden" name="studentSchool" value={1 /* Cambiar por colegio */} />
                    <div className="grid grid-cols-2 gap-4 border rounded-lg p-2">
                        <TextField label="Nombre y apellido" name="contactName" id="" defaultValue={currentStudent.student_contact.name}></TextField>
                        <TextField label="E-mail" name="contactEmail" id="" defaultValue={currentStudent.student_contact.email}></TextField>
                        <TextField label="telefono" name="contactPhone" id="" defaultValue={currentStudent.student_contact.phone}></TextField>
                        <div className="flex flex-col justify-center">
                            <label htmlFor="">Tipo</label>
                            <select name="contactType" id="" defaultValue={currentStudent.student_contact.type} className=" outline outline-1 outline-outline rounded p-4 w-full">
                                <option value="-">---</option>
